...
(function(){
	var RunalyzePlot = {
			...
			initSaver: function() {
				if (!this.options.saverReady) {
					$('body').append(
						...
						+'</form><iframe style="display:none;" name="isavePng" src="" width="1" height="1"></iframe>');
			...
			resizeTrainingCharts: function() {
				...
				$("#training-plots-and-map").width( $("#training-display").width() - $("#training-table").outerWidth(true) - 20 );
			...
			save: function(idOfPlot) {
				...
				if (plot.getContext) {
					...
					//bg.onload = function() {
						...
						imgx.fillStyle = obj.getOptions().grid.backgroundColor;//RunalyzePlot.options.saveBgColor;
					...
					window.alert('Sorry, your browser is not able to save images from canvas.');
				}
			}
	}
