...
RunalyzeLeaflet.Routes = (function($, parent){
    ...
    self.movePosMarker = function(step) {
        id=RunalyzeLeaflet.Routes.routeid;
        infoArr=_objects[id].segmentsInfo[0];
        for (j = 0; j < infoArr.length-1; ++j)
            if (infoArr[j]['km'] > step)
                break;
        if (!RunalyzeLeaflet.Routes.posmarker){
            RunalyzeLeaflet.Routes.posmarker = L.marker(_objects[id].segments[0][j],{icon: RunalyzeLeaflet.Routes.posIcon()}).addTo(RunalyzeLeaflet.map());
        }
        RunalyzeLeaflet.Routes.posmarker.setLatLng(_objects[id].segments[0][j]);
    };
	...
	self.addRoute = function(id, object) {
        ...
        RunalyzeLeaflet.Routes.posmarker=null;
	};
