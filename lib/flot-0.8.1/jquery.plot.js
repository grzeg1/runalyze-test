function bindFlotForQTipPoints(elem, plot) {
	...
	elem.bind('plothover', function(event, coords, item) {
        ...
        if (item) {
			...
			content = content + tipLineFor("at", axes.xaxis.tickFormatter(Math.round(x*100)/100, axes.xaxis));
...
function bindFlotForQTip(elem, plot) {
	...
	elem.bind('plothover', function(event, coords, item) {
		...
		content = content + tipLineFor("at", axes.xaxis.tickFormatter(Math.round(coords.x*100)/100, axes.xaxis));
        ...
        if (elem.attr('id').startsWith('collection'))
            RunalyzeLeaflet.Routes.movePosMarker(coords.x);
...
function bindFlotForSelection(elem, plot, rangeCalculation) {
	...
	elem.bind('plotselected', function(event, ranges) {
		...
		var i, j, dataset = plot.getData();
		for (i = 0; i < dataset.length; ++i) {
			var series = dataset[i], num = 0, sum = 0, min=0, max= 0, val= 0;

			for (j = 0; j < series.data.length; ++j)
				if (series.data[j][0] >= from && series.data[j][0] <= to){
					sum = sum + series.data[j][1];
					num = num + 1;
                    val=parseInt(series.data[j][1]);
                    if (num==1||val<min) min=val;
                    if (val>max) max=val;
				}
            if (series.label=='Pace') paceseries=series;

			if (series.label=='Time'){
                content = content + tipLineFor(paceseries.label, "&oslash; "+paceseries.yaxis.tickFormatter(Math.round((max-min)/(to-from)), paceseries.yaxis));
                content = content + tipLineFor(series.label, series.yaxis.tickFormatter(max-min, series.yaxis));
            }
            else
            if (series.label!='Pace')
