...
class TrainingPlotPulse extends TrainingPlot {
	...
	static public function setPropertiesTo(Plot &$Plot, $YAxis, TrainingObject &$Training, array $Data) {
		...
		$Plot->addMarkingArea('y'.$YAxis, HF_REST+(HF_MAX-HF_REST)*1,   HF_REST+(HF_MAX-HF_REST)*0.9, 'rgba(255,0,0,0.3)');
		$Plot->addMarkingArea('y'.$YAxis, HF_REST+(HF_MAX-HF_REST)*0.9, HF_REST+(HF_MAX-HF_REST)*0.8, 'rgba(255,255,0,0.3)');
		$Plot->addMarkingArea('y'.$YAxis, HF_REST+(HF_MAX-HF_REST)*0.8, HF_REST+(HF_MAX-HF_REST)*0.7, 'rgba(0,255,0,0.3)');
		$Plot->addMarkingArea('y'.$YAxis, HF_REST+(HF_MAX-HF_REST)*0.7, HF_REST+(HF_MAX-HF_REST)*0.6, 'rgba(0,0,255,0.1)');
	}
}
