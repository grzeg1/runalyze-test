...
class TrainingPlotSplits extends TrainingPlot {
	...
	protected function initData() {
		if ($this->splitsAreNotComplete() || !$this->Training->hasArrayDistance() || !$this->Training->hasArrayTime()) {
			...
			$this->achievedPace = $this->Training->GpsData()->getAveragePace();
			...
			foreach ($RawData as $key => $val) {
				$km = $key + 1;
				if ($num < 20) {
					$label = ($km%1 == 0 && $km > 0) ? $km.' ' : '';
				} elseif ($num < 50) {
					$label = ($km%1 == 0 && $km > 0) ? $km.' ' : '';
				} elseif ($num < 100) {
					$label = ($km%10 == 0 && $km > 0) ? $km.' ' : '';
				} else {
					$label = ($km%50 == 0 && $km > 0) ? $km.' ' : '';
				}
	...
	protected function setProperties() {
		if (!empty($this->Data)) {
			$min = 0;//Helper::floorFor(min($this->Data), 30000);
		...
		if ($this->achievedPace > 0) {
			...
			$this->Plot->addAnnotation(0, $this->achievedPace*1000, '&oslash; '.Time::toString(round($this->achievedPace)), -55, -12);
		}
	}
