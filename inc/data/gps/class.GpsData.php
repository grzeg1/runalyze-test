...
class GpsData {
	...
	*/
	...
	public function getAveragePaceOfStep() {
        if (!$this->getDistanceOfStep()) return 0;
		return round($this->getTimeOfStep()/$this->getDistanceOfStep());
	}
    ...
    public function getPulseZonesBy5() {
        ...
        while ($this->nextStep()) {
            $zone = 5*ceil(100 * ($this->getAverageHeartrateOfStep() + 1 - Helper::getHFrest()) / (Helper::getHFmax() - Helper::getHFrest())/ 5);
            if($zone<5) $zone=5;
...
//			echo ($this->arrayForDistance[$i]-$this->arrayForDistance[$j])." ".$j." ".$i." ".$bestdistance."<br/>";
	...
	public function getBestSegments() {
		...
		if ($this->getTotalTime()>12*60) $segments[$this->getBestDistanceFor(12*60).'']=12*60;
		if ($this->getTotalDistance()>1*0.985) $segments['1'] = $this->getBestTimeFor(1);
        if ($this->getTotalDistance()>3*0.985) $segments['3'] = $this->getBestTimeFor(3);
        if ($this->getTotalDistance()>5*0.985) $segments['5'] = $this->getBestTimeFor(5);
        if ($this->getTotalDistance()>10*0.985) $segments['10'] = $this->getBestTimeFor(10);
    ...
    public function getPlotDataForTime() {
        return $this->getPlotDataFor('time');
    }
