...
class RunalyzePluginPanel_Rechenspiele extends PluginPanel {
	...
	protected function getRightSymbol() {
		$Links = '';
		$Links .= '<li>'.Ajax::window('<a href="plugin/'.$this->key.'/window.plot.php?y=all" '.Ajax::tooltip('', __('Show shape'), true, true).'>'.Icon::$FATIGUE.'</a>', 'big').'</li>';
	...
	protected function showValues() {
		...
		$Values = array(
			...
			array(
				...
				'bars'	=> array(
					new ProgressBarSingle($ATLabsolute, ProgressBarSingle::$COLOR_BLUE)
				),
			...
			array(
				'show'	=> $this->config['show_trimpvalues']['var'],
				'bars'	=> array(
					new ProgressBarSingle($CTLabsolute, ProgressBarSingle::$COLOR_BLUE)
				),
	...
	public function getFieldsetVDOT() {
		...
		$VDOTs = DB::getInstance()->query('SELECT `id`,`time`,`distance`,IF(`vdot_with_elevation`>0,`vdot_with_elevation`,`vdot`) as vdot FROM `'.PREFIX.'training` WHERE time>='.(time() - CONF_VDOT_DAYS*DAY_IN_S).' AND vdot>0 AND use_vdot=1 ORDER BY time ASC')->fetchAll();
